...
def upload_vid_frames():
    if request.method == "POST" and "vid_file" in request.files:
        ...
        # Secure a filename
        fn = secure_filename(
            "".join(random.choices(string.ascii_lowercase, k=10)) + ".mp4"
        )
...
def encode_frame(base64_frame):
    ...
    # rgb2bgr was needed for mp4, but not webm
...
def get_dominant_emotion(data_dict):
    emotion_count = {'anger': 0, 'contempt': 0, 'disgust': 0, 'engagement': 0, 'fear': 0, 'joy': 0, 'sadness': 0, 'surprise': 0}
